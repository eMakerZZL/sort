...
void test_bubble_sort_randomElemWillInCreaseSort(void)
{
    const elem_type output_arr[] = { 1, 2, 3, 4, 5 };

    bubble_sort(input_arr, arr_length, increase);

    TEST_ASSERT_EQUAL_UINT32_ARRAY(output_arr, input_arr, arr_length);
}

void test_bubble_sort_randomElemWillDeCreaseSort(void)
{
    const elem_type output_arr[] = { 5, 4, 3, 2, 1 };

    bubble_sort(input_arr, arr_length, decrease);
